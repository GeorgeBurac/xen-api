...
let test_create_internal () =
  ...
  let sriov, logical_PIF =
    Xapi_network_sriov.create_internal ~__context ~physical_PIF ~physical_rec
      ~network
  in
  Alcotest.check
    Alcotest_comparators.(ref ())
    "test_create_internal sriov_ physical_PIF" sriov
    (List.hd (Db.PIF.get_sriov_physical_PIF_of ~__context ~self:physical_PIF)) ;
  Alcotest.check
    Alcotest_comparators.(ref ())
    "test_create_internal sriov_logical_PIF" sriov
    (List.hd (Db.PIF.get_sriov_logical_PIF_of ~__context ~self:logical_PIF)) ;
  Alcotest.check
    Alcotest_comparators.(ref ())
    "test_create_internal physical_PIF" physical_PIF
    (Db.Network_sriov.get_physical_PIF ~__context ~self:sriov) ;
  Alcotest.check
    Alcotest_comparators.(ref ())
    "test_create_internal logical_PIF" logical_PIF
    (Db.Network_sriov.get_logical_PIF ~__context ~self:sriov) ;
  Alcotest.check
    Alcotest_comparators.(ref ())
    "test_create_internal network" network
    (Db.PIF.get_network ~__context ~self:logical_PIF) ;
  Alcotest.check
    Alcotest_comparators.(ref ())
    "test_create_internal host" host
...
let test_create_on_bond_master () =
  ...
  in
  let network = T.make_network ~__context ~bridge:"xapi0" () in
  Alcotest.check_raises "test_create_on_bond_master"
...
let test_create_on_vlan () =
  ...
  in
  let network = T.make_network ~__context ~bridge:"xapi0" () in
  Alcotest.check_raises "test_create_on_vlan"
    Api_errors.(Server_error (pif_is_not_physical, [Ref.string_of pif]))
    (fun () -> Xapi_network_sriov.create ~__context ~pif ~network |> ignore)
...
let test_create_on_pif_not_have_sriov_capability () =
  ...
  let network = T.make_network ~__context ~bridge:"xapi0" () in
  Alcotest.check_raises "test_create_on_pif_not_have_sriov_capability"
...
let test_require_operation_on_pci_device_not_attached_not_need_reboot () =
  ...
  let sriov_logical_PIF, sriov =
    ...
    let sriov_logical_PIF =
      T.create_sriov_pif ~__context ~pif:physical_PIF ~network ()
    in
    Db.PIF.set_currently_attached ~__context ~self:sriov_logical_PIF
      ~value:false ;
    let sriov =
      List.hd
        (Db.PIF.get_sriov_logical_PIF_of ~__context ~self:sriov_logical_PIF)
    in
...
let test_require_operation_on_pci_device_unknown () =
  ...
  let sriov_logical_PIF, sriov =
    ...
    in
    Db.Network_sriov.set_requires_reboot ~__context ~self:sriov ~value:false ;
    Db.Network_sriov.set_configuration_mode ~__context ~self:sriov
      ~value:`unknown ;
    (sriov_logical_PIF, sriov)
  in
  Alcotest.(check bool)
...
let create_physical_pif_with_driver ~__context ~host ~network
  ...
  let physical_PIF = T.create_physical_pif ~__context ~host ~network () in
  let pci =
    T.make_pci ~__context ~vendor_id:"99" ~device_id:"1" ~driver_name ()
  in
...
let test_require_operation_on_pci_device_modprobe_0 () =
  ...
  let sriov_logical_PIF, sriov =
    ...
    let network = T.make_network ~__context ~bridge:"xapi0" () in
    let physical_PIF =
      create_physical_pif_with_driver ~__context ~host ~network ()
    in
    let sriov_logical_PIF =
      T.create_sriov_pif ~__context ~pif:physical_PIF ~network ()
    in
...
let create_modprobe_sriov_logical_pif_with_driver ~__context ~host ~network
  ...
  let physical_PIF =
    create_physical_pif_with_driver ~__context ~host ~network ~driver_name ()
  in
  let sriov_logical_PIF =
    T.create_sriov_pif ~__context ~pif:physical_PIF ~network ()
  in
  ...
  let sriov =
    List.hd (Db.PIF.get_sriov_logical_PIF_of ~__context ~self:sriov_logical_PIF)
  in
...
let test_require_operation_on_pci_device_modprobe_2 () =
  ...
  let _ =
    let network = T.make_network ~__context ~bridge:"xapi0" () in
    create_modprobe_sriov_logical_pif_with_driver ~__context ~host ~network
      ~driver_name ~currently_attached:false ~requires_reboot:false
  in
  let sriov_logical_PIF, sriov =
    let network = T.make_network ~__context ~bridge:"xapi1" () in
    create_modprobe_sriov_logical_pif_with_driver ~__context ~host ~network
      ~driver_name ~currently_attached:true ~requires_reboot:false
  in
  Alcotest.(check bool)
...
let test_require_operation_on_pci_device_modprobe_4 () =
  ...
  let sriov_logical_PIF, sriov =
    let network = T.make_network ~__context ~bridge:"xapi1" () in
    create_modprobe_sriov_logical_pif_with_driver ~__context ~host ~network
      ~driver_name ~currently_attached:false ~requires_reboot:true
  in
  Alcotest.(check bool)
...
let () =
  ...
  Debug.log_to_stdout () ;
  Alcotest.run "Test Network SRIOV suite" ["Test_network_sriov", test]
