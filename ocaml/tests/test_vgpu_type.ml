  ...
  module HostDriverMultipleVgpuSupport = Generic.MakeStateless (struct
    ...
    let tests =
      `QuickAndAutoDocumented
        ...
        ; (* Test specific version should match *)
          (("430.19", ["430.19"; "typo_error"]), true)
        ; (* Test specific version should match *)
          (("430.19", ["430.19typo"]), false)
        ; (* Test specific version with typo should not match *)
          (("430.19", ["430.18"]), false)
        ...
        ; (* Lower version should not match *)
          (("530.400", ["430.200+"; "typo_error"]), true)
          (* One wrong format config should not affact others *)
        ]
  end)
end
